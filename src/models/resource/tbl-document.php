...
class TblDocument extends SqlTable
{
    ...
    public static function update($document_id, $title, $desciption, $price)
    {
        $tname = self::$table_name;
        $sql = "
            UPDATE $tname
        ...
        ;";
        $conn = self::get_connection();
        $conn->prepare($sql)->execute([$title, $desciption, $price, $document_id]);
    }
    ...
    public static function delete($document_id)
    {
        $tname = self::$table_name;
        $sql = "
            delete from $tname where doc_id = ?;
            delete from tblRequestDocument where doc_id = ?;
        ";
        $conn = self::get_connection();
        $ex = $conn->prepare($sql);
        $ex->execute([$document_id, $document_id]);
    }
